...
class SWAGInference(object):
    ...
    def __init__(
        ...
        self.n_models = 0
        self.swag_first_moment = self._create_weight_copy()
    ...
    def update_swag(self) -> None:
        ...
        for name, param in current_params.items():
            ...
            self.swag_first_moment[name] = (self.n_models * self.swag_first_moment[name] + param) / (self.n_models + 1)
            self.swag_second_moment[name] = (self.n_models * self.swag_second_moment[name] + param**2) / (self.n_models + 1)
        ...
        if self.inference_mode == InferenceMode.SWAG_FULL:
            ...
            for name, param in current_params.items():
                if len(self.swag_deviation[name]) == self.deviation_matrix_max_rank:
                    self.swag_deviation[name].popleft()
                self.swag_deviation[name].append(param - self.swag_first_moment[name])
        ...
        with tqdm.trange(self.swag_epochs, desc="Running gradient descent for SWA") as pbar:
            ...
            for epoch in pbar:
                ...
                if (epoch+1) % self.swag_update_freq == 0:
                    self.n_models = int((epoch+1) / self.swag_update_freq)
                    self.update_swag()
    ...
    def predict_probabilities_swag(self, loader: torch.utils.data.DataLoader) -> torch.Tensor:
        ...
        for _ in tqdm.trange(self.bma_samples, desc="Performing Bayesian model averaging"):
            ...
            for (batch_xs,) in loader:
                batches_aggregate.append(self.network(batch_xs))
            per_model_sample_predictions.append(torch.cat(batches_aggregate))
        ...
        bma_probabilities = torch.softmax(torch.stack(per_model_sample_predictions), dim=-1) #torch.mean(torch.stack(per_model_sample_predictions), dim=0)
        bma_probabilities = torch.mean(bma_probabilities, dim=0) #torch.softmax(bma_probabilities, dim=-1)
    ...
    def sample_parameters(self) -> None:
        ...
        for name, param in self.network.named_parameters():
            ...
            current_mean = self.swag_first_moment[name]
            current_std = (self.swag_second_moment[name] - self.swag_first_moment[name]**2)**0.5
            ...
            if self.inference_mode == InferenceMode.SWAG_FULL:
                ...
                low_rank_std = torch.zeros(param.size())
                for i in range(self.deviation_matrix_max_rank):
                    # Instead of querying the deque for each index i,
                    # we pop the leftmost element and re-append it
                    # to the end of the deque, making each access faster
                    # (O(1) instead of O(n) each time)
                    deviation_elem = self.swag_deviation[name].popleft()
                    low_rank_std += z_2[i] * deviation_elem
                    self.swag_deviation[name].append(deviation_elem)

                sampled_param += (0.5**0.5 - 1) * current_std * z_1 + (1 / (2 * (self.deviation_matrix_max_rank - 1))**0.5) * low_rank_std
...
class SWAGScheduler(torch.optim.lr_scheduler.LRScheduler):
    ...
    def calculate_lr(self, current_epoch: float, old_lr: float) -> float:
        ...
        if self.min_lr < self.max_lr and min(self.min_lr, self.max_lr) > 0:
            p = current_epoch - int(current_epoch)
            return (1 - p) * self.max_lr + p * self.min_lr
        return old_lr
